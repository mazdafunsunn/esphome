...
void HOT WaveshareEPaper::draw_absolute_pixel_internal(int x, int y, int color) {
  ...
  int width_padding = (8 - this->get_width_internal() % 8) % 8 // add this for next multiple of 8
...
void WaveshareEPaperTypeA1::initialize() {
  // COMMAND DRIVER OUTPUT CONTROL
  this->command(0x01);
  this->data(this->get_height_internal() - 1);
  this->data((this->get_height_internal() - 1) >> 8);
...
void HOT WaveshareEPaperTypeA1::display() {
  if (!this->wait_until_idle_()) {
    this->status_set_warning();
    return;
  }
